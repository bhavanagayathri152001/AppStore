...
class AppStore extends Component {
  ...
  getUpdateAppItems = () => {
    ...
    const filteredAppResults = appsList.filter(
      eachItem => eachItem.category === activeTabId,
    )
    return filteredAppResults
  }

  render() {
    const {searchInput, activeTabId} = this.state
    const filteredAppResults = this.getUpdateAppItems()

    const searchResults = filteredAppResults.filter(eachResult =>
